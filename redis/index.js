...
exports.debug_mode_redis = false;
...
RedisClient.prototype.install_stream_listeners = function() {
    ...
    this.stream.on("close", function () {
        // @muchao 退出时需要把所有nodes都关闭
        for(var redis_key in nodes){
            node = nodes[redis_key];
            node.quit();
        }
        /// end of muchao
...
RedisClient.prototype.return_error = function (err) {
    ...
    // @muchao 
    // 返回错误时，如果错误信息是MOVED，需要重新链接一个redis实例，重新执行一次查询
    if(exports.debug_mode_redis){
        console.log(err);
    ...
    if(tmp[0] == 'MOVED'){
        ...
        redis = nodes[tmp[2]];
        // console.log(';;;;;;');
        // console.log(redis instanceof RedisClient);
        if( !(redis instanceof RedisClient)){
            // nodes里面不存在，新创建
            if(exports.debug_mode_redis){
                console.log('======create new redis======');
            }
            var address = tmp[2].split(":");
            // console.log(address);
            redis = createClient_tcp(address[1], address[0]);
            nodes[tmp[2]] = redis;
            // console.log(nodes);
        } else {
            // nodes里面已经存在，直接使用
        ...
        var callback,
            command_callback = command_obj.callback;
        var args = command_args[0];

        if(command_args.length > 1){
            callback = command_args[1];
        }
        if(exports.debug_mode_redis){
            console.log(command);
            console.log(args);
        ...
        if (Array.isArray(args) && typeof command_callback === "function") {
            if(exports.debug_mode_redis){
                console.log('======has callback=======');
            }
            return redis.send_command(command, args, command_callback);
        } else {
            // 这里不能用arguments了，因为command中的args已经是被处理过了，所以可以直接拿来用
            if(exports.debug_mode_redis){
                console.log('======no callback=======');
                console.log(callback);
            }
            last_arg_type = typeof command_callback;
            if(last_arg_type == 'function' || last_arg_type == 'undefined'){
                //command_args[command_args.length] = command_callback;
                command_args.push(command_callback);
            }
            return redis.send_command(command, command_args);
        }
    }
    ///end of muchao
...
commands.forEach(function (fullCommand) {
    ...
    RedisClient.prototype[command] = function (args, callback) {
        if(exports.debug_mode_redis){
            console.log('======command call args======');
            console.log(command);
            console.log(args);
            console.log(callback);
            console.log(arguments);
            console.log('======command call end======');
        }
        if (Array.isArray(args) && typeof callback === "function") {
            // @muchao
            // 这里只能是调用包含2个参数，且第一个是数组，第二个是function的方式
            return this.send_command(command, args, callback);
        } else {
            // @muchao
            // 这里会把所有参数都当做一个数组传进来，如redis.set('key', 'value', function(){});
            // 这里会把参数列表传到send_command的第二个参数中
...
//@muchao 重新定义了keys的实现，遍历所有nodes拿到参数，非常慢，不建议使用
RedisClient.prototype.keys = function(pattern, callback){
    var self = this;
    console.log('不建议使用');
    var keys = [];
    this.cluster('nodes', function(err, reply){
        var cluster_nodes = [];
        // console.log(reply);
        cluster_nodes = reply.split("\n");
        // console.log(cluster_nodes);
        for(var node in cluster_nodes){
            // console.log(cluster_nodes[node]);
            if(cluster_nodes[node] == '' || cluster_nodes[node]=='undefined'){
                continue;
            }
            var config = cluster_nodes[node].split(' ');
            var host = config[1].split(':');
            // console.log(host);
            redis = createClient_tcp(host[1], host[0]);
            // console.log(redis);
            // console.log(keys);
            // console.log('---');
            redis.send_command('keys', [pattern], function(err, reply){
                // console.log(keys);
...
// @muchao 获取集群所有节点的配置
RedisClient.prototype.nodes = function(callback){
    var self = this;
    this.cluster('nodes', function(err, reply){
        var cluster_nodes = [],
            result = [];
        ...
        for(var node in cluster_nodes){
            ...
            var config = cluster_nodes[node].split(' ');
            var host = config[1].split(':');
            result.push({'host':host[0], 'port':host[1]});
            callback(err, result);
            // console.log(host);
        }
    });
